...
import type { DpiLaboratoryRequest } from "../../types/laboratory";
import type { DpiDateInterval } from "../../types/summary";

type LabLoader<T> = (options: DpiLaboratoryRequest) => Promise<T>;

export function useDpiLabCardData<T>(load: LabLoader<T>, fallbackMessage: string) {
  const { getToken } = useAuth();
  const [interval, setInterval] = useState<DpiDateInterval>(() => getDefaultReportDateInterval());
...
export function formatDpiLabNumber(value: number) {
