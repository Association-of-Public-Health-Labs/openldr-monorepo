...
export function prepareChartDataForExcel(
  ...
  // Get drug label for display
  const drugLabels = {
    "Rifampicin": "Rifampicina",
    "Isoniazid": "Isoniazida",
    "Fluoroquinolona": "Fluoroquinolona",
    "Kanamicin": "Kanamicina",
    "Amikacin": "Amicacina",
    "Capreomicin": "Capreomicina",
    "Ethionamide": "Etionamida"
  };

  const selectedDrugLabel = drugLabels[reportState.selectedDrug] || reportState.selectedDrug;

  // Helper function to get drug data safely
  const getDrugData = (item: any, drug: string, type: string) => {
    const drugData = item[drug];
    if (drugData && typeof drugData === 'object' && type in drugData) {
      return drugData[type] || 0;
    }
    return 0;
  };
  ...
  return reportState.data.map(item => ({
    ...
    "Medicamento": selectedDrugLabel,
    "Resistente": getDrugData(item, reportState.selectedDrug, 'Resistance_Detected'),
    "Sensível": getDrugData(item, reportState.selectedDrug, 'Resistance_Not_Detected'),
    "Indeterminado": getDrugData(item, reportState.selectedDrug, 'Resistance_Indeterminate'),
