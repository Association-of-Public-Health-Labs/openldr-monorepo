...
function validateExportData(data: ExcelData[]): boolean {
  ...
  return data.every(item =>
    ...
    typeof item.goodLabel === 'string' &&
    typeof item.badLabel === 'string' &&
    typeof item.good === 'number' &&
    typeof item.bad === 'number' &&
...
function prepareChartDataForExcel(
  ...
  const goodLabel = data[0].goodLabel;
  const badLabel = data[0].badLabel;
  ...
  const totals = [
    'TOTAL',
    data.reduce((sum, item) => sum + item.good, 0),
    data.reduce((sum, item) => sum + item.bad, 0),
...
function applyWorksheetFormatting(
  ...
  // Merge subtitle cells (A2:D2)
  ws['!merges'].push({ s: { r: 1, c: 0 }, e: { r: 1, c: 3 } });

  // Merge interval type cells (A3:D3)
